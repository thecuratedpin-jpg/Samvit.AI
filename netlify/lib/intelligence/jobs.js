...
export async function createJob(accountId,body,env){
 ...
 // P1: a mission may target a specific paired computer. Resolve and verify it
 // NOW rather than discovering mid-mission that it is missing or revoked.
 let deviceId=null;
...
export async function commandJob(accountId,id,action,payload={}){
 ...
 const {value}=await casUpdate(getStore(JOB_STORE),jobKey(accountId,id),r=>{if(!r||r.accountId!==accountId)throw Error('Job not found');
  ...
  if(action==='decide'){
   ...
   return {...r,decisions:{...(r.decisions||{}),[decisionId]:answer.trim()},pendingDecision:null,status:'queued',sessionVersion:account.sessionVersion,error:null,notification:null,updatedAt:Date.now()};
  }
